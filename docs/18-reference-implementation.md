...
| 03 Roles | `src/manifestmodel.{h,cpp}`, `src/pkggraph.cpp` | `Node::IsLaunchable/IsRunner/Presentable/GameKey/ExecFor`, `DeriveIdentity` (own `TILE.UID` else the union over `OVER`); `PkgGraph::AllTypes/FieldsFor` (the declared per-section field table) |
| 02 The edge | `src/manifestmodel.{h,cpp}` | `OverReq`, `ParseOver`, `OverRefs`, `RemapOverRefs` — the one reader and the one remapper of `OVER` (freeze and `StampNodeCids` share it) |
...
| 12 Resolution | `src/manifestmodel.cpp`, `src/launchresolver.cpp` | `ResolveNodeOrder` (the closure; any-of groups deferred, `NOT` first-kept-wins), `Reachable`, `OfferedGrafts` (judged against the SELECTED set, fixpoint), `ResolveGraftOrder` (above the closure, by `ContainerParams::GraftPrecedence`), `OptionalNodes`, `ForEachClosureNode`; the launch runs `Launch->ExecFor(CP.Entrypoint)` — never an entry under it |
