...
| **A delta has something below it** | warning | `FORM: "delta"` reconstructs against the composed view at its own `TARGET` (or at `BASE_TARGET`, when it names a different one). A delta that is the LOWEST layer at its target has nothing to reconstruct against; warn, since whether a base arrives from another node's closure cannot always be decided statically. |
