...
|-------|-----------|---------|---------|
| `LABEL` | string | `""` (optional) | A pretty, human name (`"Age of Empires II — Retail Clean Install"`), shown in every UI. It also serves as the node's **intra-tree authoring handle**: `PARENTS`/`LIBRARYITEM`/`EXCLUDE`/`RUNNER` reference a sibling by its `LABEL`, resolved to a CID at freeze. It travels in the frozen block, so a receiver shows the name with no re-labeling. It MUST be unique **within one authoring tree** where used as a handle (invariant **I1**); across bundles/authors it may repeat freely — identity is the CID, never the label. Editing a `LABEL` changes the node's CID (it is part of the block). A node MAY omit `LABEL` (then it is unnamed, referenceable only by CID). |
...
| `PARENTS` | array of string | `[]` | The nodes this node depends on — sibling `LABEL`s in an authoring tree, CIDs once frozen. Every parent is pulled into the closure, and every parent is applied **before** this node — that is how layer order survives the flattening. A parent id that names no node in the index is a **dangling reference** and MUST be an error. |
...
| `EXCLUDE` | array of string | `[]` | nodes (by `LABEL`) this node is mutually exclusive with. **Symmetry is expected** (both nodes should list each other); validators warn if not. When two excluded nodes would both be enabled, first-kept wins. See [ch. 12 §12.4](12-resolution.md). |
